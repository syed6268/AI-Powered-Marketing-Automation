...
#                 "Platforms to run ads, give three platforms where i can market this company so that it reaches the target audience with urls and description about how the platform can help your specific company "
#                 "Your task is to find three companies that can assist this company in enhancing its operational efficiency. this coompany is looking for service providers that specialize in helping them operate effiicenty. Your recommendations should be mindful of the given company's valuation and its affordability for these services."
#                 "Marketing tips, tailored to this company, give me three "
...
#         "For Blog Newsletter Recommendations, give me three blogs that talk about the same objective, where I can market this company. "
...
#         "Find three companies that can assist Grad Stem in enhancing its operational efficiency. this company is looking for service providers that specialize "
#         "in helping them operate efficiently. Your recommendations should be mindful of this company valuation and its affordability for these services. "
#         "Marketing tips tailored to  the given company, give me three. Only JSON, no other things."
#     )
...
def get_perplexity_responses(summary):
    ...
    user_message = (
        ...
        "Platforms to run ads, give three platforms where I can market t so that it reaches the target audience with URLs and a description "
        "about how the platform can help your specific company.the keys should be name, url, description "
        "Find three companies that can assist this company in enhancing its operational efficiency. this company is looking for service providers that specialize "
        "in helping them operate efficiently. Your recommendations should be mindful of this company valuation and its affordability for these services. "
        "Marketing tips tailored to this company, give me three. Only JSON, no other things."
        '''{
  "Influencers": [
    {
      "name": "",
...
'''
    ...
    try:
        ...
        if response.status_code == 200:
            response_json = response.json()
            content = response_json['choices'][0]['message']['content']  # Extract the content part
            
            # Clean up the content by removing ```json and ``` markers
            if content.startswith('```json'):
                content = content.replace('```json', '').replace('```', '').strip()
            
            # Convert the cleaned string content back to a JSON object
            cleaned_response = json.loads(content)
            return cleaned_response  # Return the cleaned and parsed JSON
        else:
            return {"error": f"Error {response.status_code}: {response.text}"}  # Return error details
